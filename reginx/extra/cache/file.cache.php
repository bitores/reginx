 ...
 * $Id: file.cache.php 220 2015-01-27 10:01:07Z reginx $
 */
class file_cache extends cache {
    ...
    public function __construct ($conf = array()) {
        ...
        $this->_dir = $this->_dir . APP_NAME . DS;
        if (!is_dir($this->_dir)) {
            core::mkdir($this->_dir);
        }
    }
    ...
    public function flush ($group = null) {
        if (empty($group)) {
            return core::rmrf(CACHE_PATH . APP_NAME);
        }
        else {
            return core::rmrf(CACHE_PATH . APP_NAME . DS . $this->_getdir($group));
        }
    }
}