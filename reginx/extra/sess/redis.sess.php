 ...
 * $Id: redis.sess.php 8569 2015-11-26 10:01:00Z reginx $
 */
class redis_sess extends sess {
    ...
    public function __construct ($conf, $sess_name = null, $sess_id = null) {
        if (!class_exists('Redis', false)) {
            throw_error(LANG('does not support', 'Redis'), 'DNS');
        }
        ...
        if ($this->_redis->connect($conf['host'], $conf['port'])) {
            ...
            throw_error(LANG('connect to server failure', 'Redis'), 'DSCF');
        }
        ...
        if (!preg_match('/^RS\d{1,3}\-[\w\-]+$/i', $this->_uuid)) {
			// 新的 session 
            $this->_uuid = uniqid('RS' . substr($_SERVER['SERVER_ADDR'], strrpos($_SERVER['SERVER_ADDR'], '.') + 1) . '-', true) 
                                . mt_rand(0, 999999);
            // compatible default naming rules
