...
class reginx {
    ...
    public function __construct() {
        ...
        // 缓存操作对象
        $GLOBALS['_CACHE'] = cache::getobj(isset($GLOBALS['_RC']['cache']) ? $GLOBALS['_RC']['cache'] : array());

        // 模板操作对象
        $GLOBALS['_TPL'] = template::getobj(isset($GLOBALS['_RC']['tpl']) ? $GLOBALS['_RC']['tpl'] : array());
    ...
    private function _initenv () {
        ...
        // router init
        core::routerparse($GLOBALS['_RC']['router']);
        // app url
        define('APP_URL', core::getappurl());
        define('BASE_URL', core::get_base_url());
        define('DATA_URL',  BASE_URL . 'data/');
        define('UPLOAD_URL', isset($GLOBALS['_RC']['upload_url']) ? $GLOBALS['_RC']['upload_url'] : (DATA_URL . 'attachment/'));
        define('STATIC_URL', isset($GLOBALS['_RC']['static_url']) ? $GLOBALS['_RC']['static_url'] : (BASE_URL . 'static/'));
        // timezone
    ...
    private function _loadfiles () {
        ...
        // 生成 临时文件
            ...
            if (!is_file($runtimefile) || !file_exists($runtimefile)) {
                ...
                $files[] = REX_PATH . 'extra' . DS . 'log'   . DS . 'file.log.php';
